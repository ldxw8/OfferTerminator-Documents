...
- `三级封锁协议`：在二级的基础上，要求读取数据 A 时必须加 S 锁，直到事务结束了才能释放 S 锁。`防止读取时修改`
	...
	|![图1-6一级封锁协议示例](img/CyC2018-CS-Notes_1-6.png) |![图1-7二级封锁协议示例](img/CyC2018-CS-Notes_1-7.png) |![图1-8三级封锁协议示例](img/CyC2018-CS-Notes_1-8.png) |
	| :---: | :---: | :---: |
...
#### 隔离级别
- 概念：事务指定一个隔离级别，该隔离级别定义一个事务必须与由其他事务进行的资源或数据更改相隔离的程度。

	隔离级别是从允许并发副作用发生的角度进行描述的。例如，脏读、不可重复读或幻影读。如表 1-3 所示，是关于隔离级别与并发副作用的层级关系图。
...
- 而 `未提交读` 隔离级别总是读取最新的数据行，无需使用 MVCC。
