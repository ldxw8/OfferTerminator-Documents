...
- B+ Tree：
	...
	- B+ Tree 与 B Tree 最大区别是内部结点不保存数据，只用于索引，所有数据都保存在叶子结点中。

		| ![图3-1B+树](img/CyC2018-CS-Notes-Database_3-1.png) |
		| :---: |
		| 图 3-1 B+树 |
...
- `利用磁盘预读特性`：为了减少磁盘 I/O 操作，磁盘往往不是严格按需读取，而是每次都会 `预读`。预读过程中，磁盘进行 `顺序读取`，顺序读取不需要进行磁盘寻道，只需要很短的磁盘旋转时间，速度非常快。
